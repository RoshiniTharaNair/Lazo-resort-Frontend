...
export default function EventCalendarView() {
  const { events, filteredEvents, setFilteredEvents } = useEventCalendar();
    ...
    fetch(`${apiUrl}/resorts/view`)
      ...
      .then((data) => {
        const formattedResorts = data.map((resort: any) => ({
          resortIdentifier: resort.resortIdentifier,
          label: resort.label,
        }));
  ...
  }, [selectedResort, events, setFilteredEvents]);
  ...
  return (
    <div className="@container">
      <div className="flex items-center justify-between mb-4">
        <Select
          label="Select Resort"
          options={resorts.map(resort => ({ value: resort.resortIdentifier, label: resort.label }))}
          value={selectedResort}
          onChange={(option: ResortOption | null) => setSelectedResort(option)}
          displayValue={(option: ResortOption) => option.label}
          className="w-auto"
        />
        <div className="flex items-center gap-3">
          ...
          <ModalButton
            ...
            view={<EventForm resorts={resorts.map(resort => ({ resortIdentifier: resort.resortIdentifier, label: resort.label }))} />}
            customSize="900px"
